...
// 物理内存页面管理的栈
static uint32_t pmm_stack[PAGE_MAX_SIZE];
...
void init_pmm()
{
  mmap_entry_t *mmap_start_addr =
    (mmap_entry_t *)glb_mboot_ptr->mmap_addr;
  mmap_entry_t *mmap_end_start =
    (mmap_entry_t *)glb_mboot_ptr->mmap_addr + glb_mboot_ptr->mmap_length;
  mmap_entry_t *map_entry;

  for (map_entry = mmap_start_addr;
       map_entry < mmap_end_start; map_entry++) {
    /* 1 表示内存可用 */
    if (map_entry->type == 1 && map_entry->base_addr_low == 0x100000 ) {
      /* 把内核结束位置到结束位置的内存段,
         按页存储到页管理栈里最多支持512和物理内存MB */
      uint32_t page_addr =
        map_entry->base_addr_low + (uint32_t) (kern_end - kern_start);
      uint32_t length =
        map_entry->base_addr_low + map_entry->length_low;
